...
批次 D 恰恰用它批量生成了 35 行事件行。**按登记表自己的标注，这 35 行不可采信。**
...
**这就把 CZCE 采购清单的价值量化了**：
`docs/research/2026-08-19-czce-holiday-notice-shopping-list.md` 要的 77 个逐年公告 URL，
换来的是这 341 条的合规覆盖。在拿到它们之前，CZCE 的节前行只能停在「候选」，
不该写进资产 —— 否则等于用一条明确标注「不得派生行」的规则去造 341 行事实。
