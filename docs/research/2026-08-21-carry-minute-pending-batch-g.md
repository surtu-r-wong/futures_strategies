...
**实证：那晚只动了开盘，收尾一个没变。** 拿 2019-12-24 夜（正常）与 2019-12-25 夜（延迟）
同一批代表合约逐个对照：

| | 2019-12-24 夜 | 2019-12-25 夜 |
|---|---|---|
| 首笔成交 | 21:00 | **22:29**（＝集合竞价 22:25–22:30 那根，被竞价归位规则归到 22:30） |
| SHFE AG/AU、INE SC 收尾 | 02:29 | **02:29** |
| SHFE CU/AL 收尾 | 00:59 | **00:59** |
| CZCE MA/SR/TA、SHFE RB 收尾 | 22:59 | **22:59** |

这与 DCE 通知的措辞完全一致（「夜盘交易时间调整为 22:30—23:00」——23:00 本就是大商所的正常收尾）。
**四家是同一件事：开盘统一延到 22:30，收盘不变。**
...
我倾向 **路 B**，且上面的实证把这条从「我的偏好」变成了「通知的原话」：
四所改的都只是开盘时间，收尾各品种照旧。路 A 要为 SHFE 写 3–13 行去复述本来就没变的收尾，
是把「未改动」当成「改动」记账。路 B 的代价是新增一种通配语义，会削弱
「首尾精确匹配」这条现有的 fail-closed 性质 —— 这一点仍要你定。
...
`carry-minute-five-missing-product-days` 立过一条硬规：**不能换个有分钟数据的兄弟合约绕开**，
因为「混淆两者会把真实缺口掩盖掉」。这里必须把两件事分开：

| 问题 | 归属 | 放宽口径后 |
|---|---|---|
| 「AL 2019-12-20 夜的会话开没开？」 | **品种级事实** | **解决**。AL2003 当场作证，这是正确答案而非绕过 |
| 「AL2002 那一夜的 240 根 K 线是不是错的？」 | **合约级归档缺陷** | **依然存在，未被解决** |

AL2002 那 240 根是结算价平铺、`volume=0`。**分钟回测真读到它就是错数据**——VWAP、成交、
持仓成本全会失真。所以 AL2002 2019-12-20 夜应当**记入采购清单**（原 15 个 contract-day 之外
的第 16 个），与那 5 个 product-day 同等对待，只是它**不挡会话资产发布**。

换句话说：放宽口径把这条从「挡发布的硬阻塞」降级为「待补的数据缺陷」，不是让它消失。
同理，AG2012 2020-05-18 夜与 FG2009 2020-05-25 夜的 21:00 那根空 K 线是**真实的无成交**
（同夜其它合约有成交、价格在动，属正常流动性现象），**不是**缺陷，无需采购。
...
| 残余 | 条数 | 性质 | 挡发布？ |
|---|---:|---|---|
| 2017-03-31 DCE、2017-04-19 CZCE 整所零成交夜 | 15 | **已判定＝归档缺陷**（2026-08-21），不再需要公告 | 是 |
| 2019-12-26 SHFE/CZCE/INE 延迟夜盘 | 22 | 缺来源 **＋ schema 表达不了** | 是 |
| 2019-12-23 SHFE AL 归档缺陷 | 1 | 已查实；采集口径改动可解**发布阻塞**，但数据缺陷仍待补 | 是 |
...
1. **采集口径**（2 + 1 = 3 条）—— 你拍板放宽夜盘边界的观测口径即可解**发布阻塞**，
   **不需要外部资源**。但 AL2002 2019-12-20 夜的归档缺陷不因此消失，见下条；
   AG / FG 两例是真实的无成交，不是缺陷。
2. **缺来源**（22 条）—— 只剩 2019-12-25 晚 SHFE / CZCE / INE 三份通知，**需要你在浏览器取**。
3. **缺数据** —— 见 `2026-08-21-minute-archive-data-request.md`，共 **127 个 contract-day/night**。

### 2026-08-21 更新：2017 那两夜已判定，缺来源从 37 条降到 22 条

原先把 2017-03-31 / 2017-04-19 的 15 条列为「缺公告、性质未定」。实测判据推翻了「未定」：

**这份归档只在夜盘会话真实运行的当晚生成夜盘 K 线。** 对照 —— 2020-03-02（疫情暂停期）
与 2018-02-14（春节前夕）**全库 21:00 后 0 根 K 线**，而正常夜是 75,480 根；
两个对照日的日盘完全正常（各合约 225 根），排除「整日缺数据」。

而 2017-03-30 夜 DCE、2017-04-18 夜 CZCE **各有完整 150 根 K 线** ——
**会话确实开市**，丢的是成交。且平铺值逐个等于当日结算价（15/15 命中），与 AL2002 同一签名。

**结论**：这 15 条是归档缺陷，**不需要去找 2017 的公告**，改由数据补发解决。
真实范围比 15 条大得多：DCE 那夜 32 个合约、CZCE 那夜 36 个合约（判据＝前后两个交易夜都正常成交）。

**另一处计数更正**：三个元旦日的缺口不是 15 个 contract-day，是 **58 个** ——
缺的是**该品种全部合约的整段日盘**，不只是主力/次主力/第三档。
