...
### ⚠️→✅ 一处与 Task 12 Step 1 的差异（既有缺陷，当天晚些时候已修）

上表记录的形态里，边界查询的 `Index Cond` **没有**裸 `symbol = c.minute_symbol`，
只有 `_ts_meta_min_1 / _ts_meta_max_1` 的时间元数据条件，symbol 匹配退到 join 侧
（`Nested Loop` + `Materialize`），故估计行高达 4,470,401。该差异在改动前就存在，
当时判为「不属本次范围」。

**几小时后它就成了主线障碍**：多年采集在 2025 年的月份上撞 300 s `statement_timeout`。
实测同一批候选（2025-06，1,387 个）——**改动前 332.6 s、改动后 361.7 s**，
即那堵墙先于本次改动存在，三个新列只占 29 s。

修法是把聚合搬进 `LEFT JOIN LATERAL`，让规划器按候选逐个索引寻址：

| | 修前 | 修后 |
|---|---:|---:|
| 2025-06（1,387 候选）耗时 | 355.9 s | **0.7 s** |
| 最大估计行 | 4,470,401 | **1,847** |
| `Index Cond` 含裸 symbol | ❌ | ✅ |
| 结果 | — | **逐行完全相同** |

修后重跑本节的定点验证，2019-12-26 五个 DCE 主力仍全部 `("22:30","23:00")`、
2019-12-25 对照夜仍 `("21:00","23:00")`，一字未变。详见提交 `642f1a1`。
