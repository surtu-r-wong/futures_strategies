...
> 离线重放发现两批合起来仍留 **217** 条歧义（不是 2 条），成因是两批在授权层互相否决 ——
> **该问题已于当天拍板并实施（`11137f8`），残余降到 1 条**。
> 现在等的是数据与公告正文，不再有待决的设计问题。
...
2. **NI 2022-03-10 的 schema** —— 建议**先拿到公告正文再定**。
   ...
   **不建议**塞进 `day_only_regimes`：设计 §3.2 明文禁止单日异常进该资产。
   这条只挡 1 条歧义，不在关键路径上。
3. 批次 A/C/D 逐行过目；批次 B 三方交叉。**过目前先看批次文档顶部的三条红字** ——
   机器体检查出：批次 D 只差 8 行（2026-02-24 与 2026-04-07 两个节后首日）；
   CZCE 的 35 行全靠一个登记表标注「不得批量生成事件行」的规则型来源
   （356 条歧义仅靠它支撑，占已解决量的 3%）；DCE 2026-01-05 一行引用了 CZCE 的 URL。
   **CZCE 那 77 个逐年公告 URL 的价值由此量化：换来 341 条的合规覆盖。**
