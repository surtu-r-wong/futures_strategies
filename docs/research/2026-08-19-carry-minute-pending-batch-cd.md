...
>    `2026-04-07`（清明后）两个目标日，批次 D 没覆盖到；第 93 条是已知的 SHFE.NI。
>    → 已补成**批次 E**（`docs/research/2026-08-19-carry-minute-pending-batch-e.md`），
>    实测 C+D+E 后残余降到 1 条。批次 E 同时更正了 D 的 2026 年两处来源缺口
>    （SHFE 退化成常年日历页、DCE 退化成 CZCE 的 PDF）。
> 3. 🔴 **CZCE 的 35 行全部靠同一个规则型来源**，登记表标 `rows_derived=0` /
>    `reviewed_rule_only`。按登记表口径剔除后残余从 93 升到 449，即
>    **356 条（CZCE 341 + DCE 15）仅靠该来源支撑**。
>    取回原文核实：该 PDF 第八条**逐字写着**「法定节假日（不包含双休日）后第一个交易日
>    无夜盘交易」——**规则本身没问题，缺的是逐年日历**（35 个日期是从 SHFE/DCE/INE
>    三所前夕取并集推的）。待裁决的是「对 CZCE 什么算权威逐年日历」，三个选项见
>    重放验证文档。另有 DCE 2026-01-05 一行引用了 CZCE 的 URL（域名错配，带出 15 条）。
>
