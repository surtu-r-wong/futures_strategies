...
## ✅ Task 1-9 执行结果（2026-09-11 回填）

分支 `feature/citic-index-replica`，11 次提交，88 个新测试，每个模块逐条变异验证。

**控制臂两轮：第一轮判引擎不合格，第二轮通过。**
...
波动比 4.94/3.12 = 1.583，与记录的尺度系数 L≈1.59 吻合；按此加杠杆年化 9.47% 对官方 9.49%。
**引擎可信。**

v1 失败的三条原因，本计划初稿全部写错或漏掉，已在设计文档更正：**宇宙**（37 是示例不是名单）、
**验收线**（0.679 不是我写的 0.99）、**收益率口径**（分母是前结算价，连 `settle` 列都没拉）。
另新增偏差 ⑧（回望期均值），已实现为 `--smoothing`。

**接线阶段炸出的真缺陷**：指数序列缺基日那一行（`accumulate` 的日历由 weights∪returns 拼出，
而基日那天两者皆无）。

**五次「变异逃逸」全部查实为测试构造不判别**，不是实现错：T1 两种规则指向同一合约 /
严格历史闸两条机制互相兜底 / 窗口截断从未被测 / 复利与累加在单日同值 / 平滑分组边界未跨越。
另有两次是变异本身未施加（sed 多行模式没匹配），已改用带 `assert count==1` 的替换。

---

## Task 9（原）: 控制臂 —— 同引擎跑 025
