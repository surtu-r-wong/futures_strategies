...
## 全历史验收：卡在时段采集的歧义裁决（2026-08-29）

- 面板依赖的时段规则资产口径太窄。收窄两刀后（只留曾入池的 63 个品种；每个品种只从
  首次入池前 252 个交易日起，两刀都可证明行为中性），仍有 23,676 个品种日缺规则。
- 采集机器已从 `feature/continuous-session-backfill` 搬来（`build_audit` 已拆成宇宙
  无关核心），驱动落在 `scripts/commodity/capture_sessions.py`，发布**第三份**资产，
  Carry 与连续两份一字不动。
- **普查已干净**：170 个月、阻塞 0、边界观测缓存已落盘。唯一采不到分钟的 937 个品种日
  全是 FU 2012–2015，被「首次入池前 252 天」那一刀整段消掉。
- ⚠️ **权威采集 `rules=0 ambiguous=547`，资产未发布**。歧义集中在夜盘分批上线那几年的
  5 个品种（CZCE MA 122 / CZCE ZC 202 / DCE B 107 / SHFE BU 57），大概率少数几条规则
  就能覆盖几百天。裁决落到 `carry_minute_day_only_regimes.csv` /
  `carry_minute_session_exceptions.csv`，每条要带交易所公告直链。
- 裁完 → 重跑采集（复用缓存约 6 分钟）→ 面板 SESSION_RULES 指向新资产 → 全历史 build
  → 两条策略 CLI → 两份验收文档。
