...
**`--end` 用 `cta_carry.coverage` 现算，不要写死。** ~~不要晚于 2026-04-29~~ ——
EOD 日更链自 2026-04-29 停摆过一段（2026-08-05 实测滞后 98 天），**2026-09-15 实测已恢复，
截断日 = 2026-09-14**。截断日取五个商品交易所 `max(trade_date)` 的**最小值**而非整体 max：
大商所人工投递常晚一天，跑过它的覆盖会把该所每个品种读成信号离场。填更晚的日期不会报错，
只会安静地少一截。
...
- 列：`signal_date`（= 截断日，**核对它是不是昨天**；不是就是有交易所滞后，见上）、`product`、`contract`（持仓合约 = 主力）、
  `order_code`（**Wind 写法**：`RB2611.SHF`，郑商所年份一位 `CF701.CZC`，Wind 取数用这列）、`code_exchange`（交易所自身代码 `rb2611` / `CF701`）、`code_qmt`（迅投 `rb2611.SF` / `CF701.ZF`）、`code_ctp`（`rb2611.SHFE` / `CF701.CZCE`）——四种口径各一列，2026-09-16 用户裁决；
...
- 研报的收益率/夏普/回撤**不是**验收指标（设计文档 §2.2）；验收看规则正确、
  结果可复现、账户可对账、数据问题可审计。
- ~~上游 `futures_daily` 停摆期间，2026-04-29 之后无数据。~~ 已恢复（2026-09-15 实测截断日 2026-09-14）。
