...
**突破比的是上一同向段的整段极值**（`lastmax_1` / `lastmin_1`，登记默认
`--breakout-reference prior_extreme`，2026-09-15 用户裁决）。研报正文写的是「临时高点」，
但公式块定义了 `lastmax_1`、图 13 标注的也是它，正文那一侧没在条件里用到 ——
规格打架时取公式块＋配图。正文读法留作变体 `--breakout-reference segment`，产物标敏感性。

⚠️ 两条读法的形状不同，别混：`prior_extreme` 是**水平**条件（收盘 ≥ 上一段那个固定极值，
价格不跌回去就一直为真），`segment` 是**增量**条件（要创本段新高）。后果之一是新读法下
持续趋势里每根都算突破，D6 的 latched / literal 之分会趋同（见下一段）。

**无论哪条读法，比的都是本 bar 纳入极值之前的状态**。先更新极值再比，`Close >= High`
几乎只在收盘恰等于本 bar 最高价时成立 —— 那是在描述一根 K 线，不是突破。

**入场后锁存**，持有到初步趋势切换或拐点条件失效；拐点失效只平仓，不反手。逐 bar
重验三道闸是敏感性变体（`--signal-mode literal`），不是可选默认。
⚠️ 登记读法 `prior_extreme` 下 literal 闸**被削弱但仍然咬人**（2026-09-15 全历史实测）：
`literal_gate_closed` 25,259 次，literal/latched 换手比 1.34（旧读法 2.11），
样本内净 3.21% 对 6.69%。合成锯齿夹具上两者会产出相同结果 —— 那是夹具没有回撤到前段
极值以下的走势，**别把夹具的性质当成策略的性质**。
