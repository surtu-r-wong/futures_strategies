...
## 实施记录续（2026-09-01）：D22 跑完，计划**结案**

三条 WSL2 跑批（A 节拍扫描 / B narrow 12 点 / C wide 12 点，全部新面板 `panel_84d482d`）
2026-08-31 夜跑完，退出码全 0。结果与定稿结论见
`docs/research/2026-08-31-continuous-turnover-verdict.md` §七～§九。

### 上一节「一个还没裁的口径」已经裁完，答案是否定的

把等权再平衡**整个关掉**（`--rebalance entry`，权重在开仓时点定死）后，年化换手从
1,607 降到 **947**，就停住了 —— 剩余 **97.5%** 是研报明文写死的信号换手。947 仍是研报
表 8 隐含的 291 倍的 **3.26 倍**。四种节拍在 1.3bp 下样本内外全负。

所以三条自由度（D21 离场口径、D22 调仓节拍、D9 均线跨度）叠加后缺口仍未闭合，
按用户 2026-08-31 的裁决**定稿为量化的否定结论**，不再找新读法。

### Task 8 的端到端已被这三条跑批覆盖

12 点网格 × 两种离场 + 5 种节拍 = 29 次真实全历史跑，全部 exit 0、产出齐全
（`run.xlsx` 7 张表 + `run-summary.csv` + `run-audit.json` + `run-window.json`）。

### 新面板复核：面板缺陷不影响任何结论

24 个网格点新旧面板对照，样本内年化最大绝对差 0.88 pp、样本外 0.30 pp，**零符号翻转**。
第一至六节跑在旧面板上的结论因此成立。

### 台账两处更新

- **D22** 加了实测定稿（四种节拍的换手与 3.26 倍缺口）。
- **D21** 原文引的 2,635 / 436 是**两年探针**的数，补了出处限定并附全历史值
  （9,164 / 1,607），免得与 D22 的数互相打架。

---

## ⚠️ 实施记录更正（2026-09-01 晚）：上一节的结案**撤回**

**上一节「D22 跑完，计划结案」的否定结论是错的。** 详见研究档
`docs/research/2026-08-31-continuous-turnover-verdict.md` **§十**。

三个方法错误：

1. **换手报在了自己网格已淘汰的点上** —— 全程用 `12/26` 比 291，而 D9 预注册网格样本内
   选中的是 `20/120`。12/26 是网格最快点。
2. **两条自由度的对角线从没跑过** —— 12 点网格全跑在 `slot` 节拍、4 种节拍全跑在 `12/26`
   跨度；两条各约 1.7 倍的削减近似**相乘**，却直接断言「叠加也补不上」。
3. **最优点落在网格边界（20/120 是最长跨度、且「越长越好」）却没去探边界。**

实测（narrow + entry，1.3bp，全历史）：

| 跨度 | 年化换手 | vs 291 | 样本内 | 样本外 |
|---|---:|---:|---:|---:|
| 12/26 | 947 | 3.26× | −0.51% | −9.20% |
| **20/120（选中点）** | 455 | 1.57× | **+6.65%** | **+2.56%** |
| 30/200* | 348 | 1.20× | +9.31% | −2.61% |
| 40/240* | 310 | **1.07×** | +9.58% | −3.45% |

\* 超出预注册网格。**按本计划 §D9「网格不得事后扩张」，这两点只用于回答「换手能否够到
291」这一个诊断问题，不构成任何业绩主张。** 在此按计划要求记录扩张的理由：用户质疑换手
是否本身有问题，而跨度是研报留白、又几乎单独决定换手的参数，必须探到边界外才能判断
「研报隐含的换手在这个策略族里是否可达」。答案是**可达**。

`log(换手) = 8.478 − 0.497·log(ema_long)`，R² = 0.9988，指数 ≈ −1/2。
反解：换手 = 291 需要 EMA ≈ **47/284**。

**修正后的状态**：复刻**成立但偏弱** —— 扣掉研报自己那 1.3bp 后样本内外皆正，
波动 13.8% 对 13.48%、杠杆 2.19 对 2.5 都对得上，但收益约 1/4、回撤约 2 倍。
剩下的是**夏普缺口（信号质量）**，不是成本、杠杆或仓位管理的问题。

**一条被证伪的假设，别再查第二遍**：怀疑 §3.1 的 `Lev_ATR<1 ⇒ 平仓` 被我们从「开仓过滤」
误读成「持仓期逐 bar 离场」（那节自我定义是「决定开仓与否」，表 3 评的是开仓过滤信号，
§5.1 构建清单没有「平仓条件」这一项）。实测：narrow 的 8,461 次离场里**均线反向 77.6%、
`Lev_ATR` 仅 21.2%**；去掉该持仓期闸，信号换手只降 **10.6%**。不是这条。
