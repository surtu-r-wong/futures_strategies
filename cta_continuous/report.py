...
DECISIONS: tuple[Decision, ...] = (
    ...
    Decision(
        "D21",
        "离场用哪几道闸",
        "两条读法都跑（`--exit-gates wide|narrow`），差异进本台账；"
        "wide = D6 原样（四道任一不过即平仓），narrow = 只有 Lev_ATR<1 或均线反向才离场",
        "D6 引的原文只点名了一道：「当开仓杠杆率 Lev_ATR<1 …… 即使此时传统信号为 1，"
        "我们依然平仓操作」，并未说另外两道也会平仓。而实测里抖得最凶的恰恰是它没点名的"
        "那两道 —— 距离扩大 47.1%、ΔTNR 48.6%，每根 15 分钟 bar 近似抛硬币；"
        "**两年探针**上 wide 年化换手约 2,635 倍、1.3bp 成本 34.3%，研报自报的 23.1% "
        "在算术上到不了，narrow 下约 436 倍 / 5.7%，容得下。"
        "（全历史 12/26 上更高：wide 9,164 倍、narrow 1,607 倍；但换手须报在网格选中的 "
        "20/120 上——见下方 D22 与研究档 §十。）见 "
        "docs/research/2026-08-31-continuous-turnover-verdict.md",
    ),
    Decision(
        "D22",
        "等权权重多久重算一次",
        "`--rebalance slot|daily|monthly|entry`，默认 slot（每个事件，即「每 15 分钟"
        "计算一次」的字面读法）。节拍只挡**再平衡**，开平仓与展期任何节拍下都照常成交",
        "研报只说「满足开仓条件的品种等权分配资金」，没说多久重算权重；而 §1.6 那一档"
        "写的是「+ **开仓时点**信号强弱」，字面上更像 entry。这条是换手的另一半："
        "narrow 口径下 43% 的换手来自逐槽重算权重与杠杆漂移。"
        "⚠️ 粗节拍下等权不再随时成立（新品种进场时旧仓位不缩，总杠杆会漂），"
        "这是该读法的代价而非实现缺陷。"
        "【2026-09-01 实测】四种节拍在 12/26 跨度上：换手 1,607 / 1,035 / 954 / 947 倍。"
        "⚠️ **当日晚些时候撤回了由此得出的否定结论**：12/26 是网格最快点、且已被样本内选点"
        "淘汰，换手必须报在选中点上。在 20/120 + entry 上换手 455（研报隐含 291 的 1.57 倍），"
        "1.3bp 后样本内 +6.65%、样本外 +2.56%，**都为正**。换手随 ema_long 服从幂律"
        "（R²=0.9988，指数 ≈ −1/2），40/240 处 310 = 1.07 倍 —— 研报隐含的换手**够得着**。"
        "见 docs/research/2026-08-31-continuous-turnover-verdict.md §十",
    ),
    Decision(
        "D23",
        "研报的基线档（EMA 均线穿越）怎么复刻",
        "`--signal-tier crossover`：穿越 + 距离走阔开仓、**满仓 ±1**、没有空仓状态、"
        "反向信号出现才反手；不含 U2P 强弱，也不乘 `Lev_ATR` 与 `Mul_vol`"
        "（连 `Mul_vol` 那 252 日自举期一并不适用）。预热与全档取同一根 bar",
        "§2.1 原文：「当短均线上穿长均线且二者距离走阔时……开多仓（Signal=1）……"
        "策略没有空仓状态，在开仓之后，直到反向信号出现，则反手开仓」「每次均线穿越的"
        "过程不加以区分，均满仓开仓」。§1.6 的四行把三级改进逐档叠上去，所以基线档"
        "按定义不含其中任何一项。它只考验数据、宇宙、主力链、复权、成交价、成本六样，"
        "是把「横截面选择差异」与「数字不可复现」分开的决定性对照"
        "（研报自报 13.06% / 夏普 1.03 / Calmar 0.93 / 回撤 13.98%）",
    ),
)
